...
糖果 A/B 评测现已内置于包内，安装后即作为 `codexcomp-eval` 直接可用。它端到端度量
修复效果：在
...
显式传 `openai_base_url`，不依赖本机既有接线；结果追加写入 `<out>/results.jsonl`，
中断后重跑同一命令即断点续跑。`on` 模式的每轮 fold 明细在 systemd user unit 启用时
从其 journal 读取。脚本用 coreutils `timeout` 包裹 `codex exec`，需 Linux/WSL
（macOS 需装 coreutils）。

```bash
codexcomp &                                     # `on` 模式需要代理已在运行
codexcomp-eval -m gpt-5.5 -r xhigh -n 5         # 小矩阵
codexcomp-eval                                  # 完整 80 跑矩阵
```
