...
- **`pool.py`** —— 共享上游 `httpx.AsyncClient` 的生命周期（#12/#13 引入，自 server.py 拆出）：每个使用方对 client 持有租约——`UpstreamRounds` 构造即租、贯穿整个 fold 含全部续写轮；`PoolTimeout` 在锁内把 client 轮换到新 generation（并发失败只轮换一次），`ConnectError` 仅在该 client 无其他租约时轮换；退役 generation 绝不复用、只在自身租约归零后关闭——恢复绝不中断进行中的流或 fold，失败请求绝不重放（保住 no-double-generation）。附带脱敏的池/代理诊断（仅 scheme/host/port，绝不含凭证）。状态挂在 `app.state`，由 `build_app` 接线。上游 round 的 read timeout 为 120s——read 约束的是相邻两次收字节的间隔而非流总时长，21 轮实测健康流最长静默 30.74s。
