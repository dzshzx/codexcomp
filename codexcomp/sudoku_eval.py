...
This puzzle is deliberately harder than the candy pigeonhole one: its unique
solution needs a long chained deduction, so runs spend far more reasoning
tokens and hit the 518n-2 truncation lattice more often — a stress test for
whether continuation folding stays stable across many rounds.
...
PROMPT = """你必须完全依靠内部推理作答。不得联网搜索，不得调用任何工具，不得写代码运行，不得使用计算器、表格、脚本或外部求解器。请仔细推理，并在作答前进行两轮自检。最终只输出完整 6×6 方阵和最后的 10 位验证码，省略推理过程。
